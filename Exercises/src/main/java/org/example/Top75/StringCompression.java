...
public class StringCompression {
    ...
    public int compress(char[] chars) {
        ...
        while(i<chars.length)
        {
            ...
            if(countOfCurrChar==1)
            {
                chars[pointer++] = val;
            }
            else if (countOfCurrChar>1 && countOfCurrChar<10)
            {
                chars[pointer++] = val;
                chars[pointer++] = Character.forDigit(countOfCurrChar, 10);
            }
            else{
                chars[pointer++] = val;
                String numStr = Integer.toString(countOfCurrChar);

                for (int k = 0; k < numStr.length(); k++) {
                    chars[pointer++] = numStr.charAt(k);
                }
            }
        }
        return pointer;
